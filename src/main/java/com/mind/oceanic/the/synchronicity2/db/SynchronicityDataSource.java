...
/**
    ...
    public SynchItem create(SynchItem synchItem) {
        ...
        Log.i("dolphin", "creating synchitem2");
...
//                    "where not exists (select * from synchItemEvents where events.EventId = synchItemEvents.seEventId);";
        String sqlCmd = "select seEventId, eventDate, eventSummary, eventDetails, eventWebId from events " +
                "left outer join synchItemEvents ON eventId  =  seEventId where seSynchId = -1;";
...
//            TODO-sql to show unlinked events
    ...
    public void updateFromWeb(SynchItem synchItem) {
        ...
        if (findFromWeb(synchItem)) {
            Log.i("dolphin","updating sychitem:"+synchItem.getSynchId()+"  "+synchItem.getSynchWebId()+synchItem.getSynchSummary());
            update(synchItem);
        } else {
...
//                "';";
    ...
    public void updateFromWeb(SynchItemEvent synchItemEvent) {
        ...
        open();
//        ContentValues values = new ContentValues();
//        values.put(SynchronicityDBOpenHelper.COLUMN_SE_SYNCH_ID, synchItemEvent.getSeSynchId());
//        values.put(SynchronicityDBOpenHelper.COLUMN_SE_EVENT_ID, synchItemEvent.getSeEventId());
    ...
    public boolean findSEForWeb(SynchItemEvent synchItemEvent) {
        String sqlCmd;

        if (synchItemEvent.getSeSynchId() < 1 || synchItemEvent.getSeEventId() < 1) {
            sqlCmd = "SELECT * FROM " + SynchronicityDBOpenHelper.TABLE_SYNCH_ITEM_EVENTS +
                    " WHERE " + SynchronicityDBOpenHelper.COLUMN_SE_SYNCH_WEB_ID + " = " + synchItemEvent.getSeWebSynchId() +
                    " AND " + SynchronicityDBOpenHelper.COLUMN_SE_EVENT_WEB_ID + " = " + synchItemEvent.getSeWebEventId() +
                    ";";
        } else {
            sqlCmd = "SELECT * FROM " + SynchronicityDBOpenHelper.TABLE_SYNCH_ITEM_EVENTS +
                " WHERE " + SynchronicityDBOpenHelper.COLUMN_SE_SYNCH_ID + " = " + synchItemEvent.getSeSynchId() +
        ...
        Cursor cursor = database.rawQuery(sqlCmd, null);
        Log.i("dolphin", "after findSEForWeb se=");
        if (cursor.getCount() > 0) {
            return true;
        } else {
            return false;
    ...
    public SynchItemEvent getKeysSEFromWeb(SynchItemEvent se) {
        ...
        if (cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                Log.i("dolphin", "after cursor pull count=" + cursor.getCount());
                se.setSeSynchId(cursor.getLong(cursor.getColumnIndex(SynchronicityDBOpenHelper.COLUMN_SYNCH_ID)));
                //se.setSeSynchId(12);
                //int this2 = cursor.getColumnIndex(SynchronicityDBOpenHelper.COLUMN_SYNCH_ID);
                //long this2 = cursor.getLong(0);
                Log.i("dolphin", "after setSe synchId="+cursor.getLong(cursor.getColumnIndex(SynchronicityDBOpenHelper.COLUMN_SYNCH_ID)));
            }
        }
